  ...
  uint8_t MPU9250::readByte(uint8_t address, uint8_t subAddress)
{
  ...
  Wire.requestFrom((int)address, 1);       // Read bytes from slave register address on MPU9250 
...
{  
  ...
  Wire.endTransmission(false);       // Send the Tx buffer, but send a restart to keep connection alive
  uint8_t i = 0;
  Wire.requestFrom((int)address, (int)count);  // Read bytes from slave register address 
